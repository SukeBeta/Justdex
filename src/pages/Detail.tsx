  ...
  if (error || !pokemon) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        <div className="text-on-surface-variant text-body-lg">Pokémon not found</div>
        <Link to="/" className="text-primary underline">Back to Pokédex</Link>
      </div>
    )
  }
    ...
    <main className="w-full max-w-[1280px] mx-auto md:px-gutter">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center">
          <div className="w-full flex justify-between items-end mb-4">
            <div>
              <h1 className="text-h1 text-white capitalize">{pokemon.name}</h1>
            ...
            <div className="flex gap-2">
              {pokemon.types.map(t => (
                <span
                  key={t}
                  className="px-3 py-1 rounded-full bg-white/20 backdrop-blur-sm font-semibold text-chip-text text-white border border-white/30 capitalize"
                >
                  {t}
                </span>
              ))}
      ...
      <div className="hidden md:block pt-8">
        <div className="mb-8">
          <Link
            to="/"
            className="inline-flex items-center gap-2 text-on-surface-variant hover:text-on-surface transition-colors py-2 group"
          >
            <span className="material-symbols-outlined group-hover:-translate-x-1 transition-transform">arrow_back</span>
            <span className="text-body-md font-medium">Back to Pokédex</span>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 px-margin-mobile md:px-0">
        ...
        <div className="hidden lg:block lg:col-span-5">
          <div className="sticky top-8 flex flex-col gap-6">
            <div
              className="rounded-3xl p-8 pt-16 flex flex-col items-center justify-center relative overflow-visible h-[400px] shadow-sm"
            ...
            <div className="flex flex-col items-start gap-4">
              <h1 className="text-h1 text-on-surface capitalize">{pokemon.name}</h1>
              <div className="flex gap-2">
                {pokemon.types.map(t => (
              ...
              {pokemon.species?.flavorText && (
                <p className="text-body-lg text-on-surface-variant max-w-md mt-2">
                  {pokemon.species.flavorText}
                </p>
              )}
              <Link
        ...
        <div className={`lg:col-span-7 flex flex-col gap-6 ${!pokemon ? '' : 'mt-16 md:mt-0'}`}>
          ...
          {pokemon.species?.flavorText && (
            <div className="lg:hidden bg-surface-container rounded-xl p-4 shadow-sm mt-16">
              <div className="flex items-center gap-2 mb-2 text-on-surface-variant">
                ...
                <span className="font-bold text-label-caps">Description</span>
              </div>
              <p className="text-body-md text-on-surface">{pokemon.species.flavorText}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="bg-surface-container-lowest rounded-2xl p-6 shadow-sm border border-surface-variant flex flex-col gap-2">
              <span className="font-bold text-label-caps text-on-surface-variant uppercase tracking-wider">Height</span>
              <span className="text-h2 text-on-surface">{pokemon.height} m</span>
            </div>
            <div className="bg-surface-container-lowest rounded-2xl p-6 shadow-sm border border-surface-variant flex flex-col gap-2">
              <span className="font-bold text-label-caps text-on-surface-variant uppercase tracking-wider">Weight</span>
            ...
            {pokemon.species && pokemon.species.eggGroups.length > 0 && (
              <div className="bg-surface-container-lowest rounded-2xl p-6 shadow-sm border border-surface-variant flex flex-col gap-2 col-span-2 md:col-span-1">
                <span className="font-bold text-label-caps text-on-surface-variant uppercase tracking-wider">Egg Groups</span>
                ...
                {pokemon.abilities.map(ability => (
                  <div key={ability.name} className="border-b border-surface-variant pb-4 last:border-0 last:pb-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {ability.isHidden ? (
                        <span className="font-bold text-label-caps bg-inverse-surface text-inverse-on-surface px-2 py-1 rounded-md">Hidden</span>
                      ) : (
                        <span className="font-bold text-label-caps bg-surface-container text-on-surface-variant px-2 py-1 rounded-md">Primary</span>
                      )}
                    </div>
                    {ability.description && (
